...
def do_insert():
    for vacancy in vacancys:
        vac.update_one({"_id": vacancy["_id"]},
                         {"$set": vacancy},
                         upsert=True)


def show(min_salary):
    # objects = vac.find({"#or":[{"salary_min": {"$gt": min_salary}},
    #                           {"salary_max": {"$gt": min_salary}}]})  # строго больше
    objects = vac.find({"salary_min": {"$gt": min_salary}} or {"salary_max": {"$gt": min_salary}})  # строго больше
    for obj in objects:
        pprint(obj)
...
print("Выводим вакансии, у которых зарплата больше 100 000")
show(100000)
#vac.delete_many({}) #удалить всё
